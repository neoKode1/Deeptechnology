...
export default function AssessmentResults({ result, quoteCreated, onReset, onCreateQuote }: {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-neutral-900 border border-white/20 rounded-xl p-5">
            ...
            {alternatives.map(m => {
              ...
              return (
                <button key={m.vendor.id} onClick={() => setSelectedIdx(idx)}
                  className="flex-1 min-w-[150px] bg-neutral-900 border border-neutral-700 hover:border-white rounded-xl p-4 text-left transition-colors">
                  ...
                  <p className="text-xs text-neutral-300 mt-2">{fmt(m.estimatedUnitCost)}/unit · {fmt(m.estimatedUnitCost * recommendedUnits)} fleet</p>
                </button>
              );
            })}
          </div>
