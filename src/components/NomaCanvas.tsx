...
/**
 * Unicorn Studio SDK v2.1.5 — matches the version tag in noma_remix (1).json.
...
const SCENE_SRC = '/media/noma_remix%20(1).json';
...
/**
 * Renders the full NOMA (Remix) Unicorn Studio scene:
 *   gradient bg → robot-head image → voronoi/shatter → chromatic aberration.
 *
 ...
 */
export default function NomaCanvas({ className = '' }: { className?: string }) {
  const divRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    /** Ask the already-loaded SDK to scan the DOM and boot our scene. */
    const init = () => {
      window.UnicornStudio?.init().catch(console.error);
    };

    if (window.UnicornStudio?.isInitialized) {
      // SDK was already injected by a previous mount — just re-init.
      init();
      return () => { window.UnicornStudio?.destroy(); };
    }

    // Avoid injecting the script twice (React StrictMode runs effects twice).
